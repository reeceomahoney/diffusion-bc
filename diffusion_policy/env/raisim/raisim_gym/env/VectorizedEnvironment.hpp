...
#pragma omp parallel for schedule(auto)
        ...
        void getJointPositionErrorHistory(Eigen::Ref<EigenRowMajorMat> &jointPosErrorHistory) {
            for (int i = 0; i < num_envs_; i++)
                environments_[i]->getJointPositionErrorHistory(jointPosErrorHistory.row(i));
        }
